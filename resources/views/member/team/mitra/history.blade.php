...
<div class="d-block mb-2 fs-auto border-bottom pb-2" id="history-filter">
    <div class="row g-2">
        <div class="col-sm-6 d-flex">
            <div class="d-flex d-flex flex-wrap align-items-center justify-content-between w-100 px-3 py-2 border border-primary rounded-3 bg-primary bg-gradient text-light text-nowrap">
                ...
                <div class="text-end">
                    ...
                    <div class="fs-4 bonus-value" data-dashboard="s=mitra&t=total-purchase"></div>
                </div>
            </div>
        </div>
        <div class="col-sm-6 d-flex">
            <div class="d-flex d-flex flex-wrap align-items-center justify-content-between w-100 px-3 py-2 border border-success rounded-3 bg-success bg-gradient text-light text-nowrap">
                ...
                <div class="text-end">
                    ...
                    <div class="fs-4 bonus-value" data-dashboard="s=mitra&t=total-bonus"></div>
                </div>
            </div>
        </div>
        @php
        ...
        <div class="col-md-6">
            <div>Member</div>
            <select class="form-select select2bs4 select2-custom" id="mitra-id">
                @if (!empty($currentMitra))
                    <option value="{{ $currentMitra->id }}" selected>{{ $currentMitra->name }}</option>
                @endif
            </select>
        </div>
    </div>
...
<table class="table table-sm table-nowrap table-striped table-hover" id="table">
    <thead class="bg-gradient-brighten bg-white small">
        <tr class="text-center">
            ...
            <th class="border-start">Bonus Referral</th>
...
<script>
    let table, selectMitra, reloadTotal;
    ...
    $(function() {
        ...
        table = $('#table').DataTable({
            ...
            serverSide: true,
            ajax: {
                url: '{{ route("member.directMitra.histories.datatable") }}',
                data: function(d) {
                    d.mitra_id = selectMitra.val();
        ...
        selectMitra.select2({
            ...
            ajax: {
                url: function(params) {
                    params.current = selectMitra.val();
                    
                    return '{{ route("member.directMitra.select2") }}';
                },
