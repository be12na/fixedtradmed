@php
    $paymentItem = optional($paymentItem);
...
<tr class="item-row">
                ...
                @foreach ($itemsProduct as $row)
                    <optgroup label="{{ $row->category->name }}">
                        @foreach ($row->products as $product)
                            <option value="{{ $product->id }}" data-unit-name="{{ $product->product_unit }}" data-zone-barat="{{ $product->harga_a }}" data-zone-timur="{{ $product->harga_b }}" data-eceran-zone-barat="{{ $product->eceranZonePrice(ZONE_WEST) }}" data-eceran-zone-timur="{{ $product->eceranZonePrice(ZONE_EAST) }}" data-eceran="{{ ($product->satuan == PRODUCT_UNIT_BOX) ? 1 : 0 }}" 
                                @foreach ($zones as $zone)
                                    data-zone-{{ $zone->id }}="{{ $product->zonePriceV2($zone->id) }}" data-eceran-zone-{{ $zone->id }}="{{ $product->eceranZonePriceV2($zone->id) }}" 
                                @endforeach
                                @optionSelected($product->id, $paymentItem->product_id ?? 0)>
    ...
    <td>
        <div class="d-block" style="min-width:100px;">
            <input type="number" class="form-control payment-item-qty" min="1" step="1" name="product_qty[]" value="{{ $paymentItem->product_qty ?? 1 }}" autocomplete="off">
        </div>
    </td>
    <td>
        <div class="d-block item-unit" style="min-width:100px;">
            <div class="unit-name text-center lh-lg"></div>
        </div>
    </td>
    <td>
