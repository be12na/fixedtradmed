                ...
                <div class="col-sm-3">
                    ...
                    <select class="form-select" id="status-id">
                        ...
                        @foreach (PAYMENT_STATUS_LIST as $value => $text)
                            @php
                                if ($value == PAYMENT_STATUS_PENDING) continue;
                            @endphp
...
<script>
    let table;
    const rejectStatus = {{ PAYMENT_STATUS_REJECTED }};
    ...
    $(function() {
        table = $('#table').DataTable({
            ...
            serverSide: true,
            ajax: {
                url: '{{ route("main.payments.datatable") }}',
                data: function(d) {
            ...
            columns: [
                ...
                {data: 'transfer_status', searchable: false, orderable: false},
            ...
            footerCallback: function(row, data, start, end, display) {
                ...
                // price
                let totalPrice = api.column( 6, { page: 'current'} ).data().reduce( function (a, b) {
                    return parseInt(a) + parseInt(b);
                }, 0 );
                $( api.column( 6 ).footer() ).html(formatCurrency(totalPrice));
                // discount
                let totalDiscount = api.column( 7, { page: 'current'} ).data().reduce( function (a, b) {
                    return parseInt(a) + parseInt(b);
                }, 0 );
                $( api.column( 7 ).footer() ).html(formatCurrency(totalDiscount));

                let totalan = 0;

                $.each(data, function(k, v) {
                    console.log(v.status);
                    if (v.status == rejectStatus) {
                        if (v.sum_rejected == 1) {
                            totalan = totalan + parseInt(v.total_transfer);
                        }
                    } else {
                        totalan = totalan + parseInt(v.total_transfer);
                    }
                ...
                const formatTotal = formatCurrency(totalan);

                $( api.column( 8 ).footer() ).html(formatTotal);
                $('#total-all').html(formatTotal);
            }
        });
        ...
        //     let url = '{{ route("main.transfers.sales.download.excel") }}?start_date=' + $('#start-date').val() + '&end_date=' + $('#end-date').val() + '&branch_id=' + $('#branch-id').val() + '&bank_code=' + $('#bank-code').val() + '&status_id=' + $('#status-id').val();
