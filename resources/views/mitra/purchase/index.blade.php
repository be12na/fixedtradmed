...
<div class="d-block mb-2 fs-auto border-bottom pb-2" id="purchase-filter">
    <div class="row g-2">
        @php
...
<div class="row gx-2 gx-md-3 justify-content-md-center order-first order-md-5 fw-bold mb-2 me-0 me-md-auto flex-md-fill" id="content-summary">
...
@php
    $canCreate = hasPermission('mitra.purchase.create');
    $myDtButtons = [];
    if ($canCreate) {
        $createUrl = route('mitra.purchase.create');
        $myDtButtons[] = [
            'id' => 'btn-tambah',
            'html' => 'Tambah',
...
<script>
    ...
    $(function() {
        table = $('#table').DataTable({
            ...
            serverSide: true,
            ajax: {
                url: '{{ route("mitra.purchase.datatable") }}',
                data: function(d) {
            ...
            columns: [
                ...
                {data: 'purchase_status', className: 'dt-body-center'},
                {data: 'view', searchable: false, orderable: false, className: 'dt-body-center'},
            ],
            // rowCallback: function(row, data) {
            //     $('td:eq(3)', row).html(formatCurrency(data.total_transfer));
            // },
            footerCallback: function(row, data, start, end, display) {
                const api = this.api();
                // omzet
                let totalOmzet = api.column( 2, { page: 'current'} ).data().reduce( function (a, b) {
                    return parseInt(a) + parseInt(b);
                }, 0 );
                $( api.column( 2 ).footer() ).html(formatCurrency(totalOmzet));
                // profit all
                let totalOmzetAll = api.column( 2 ).data().reduce( function (a, b) {
                    return parseInt(a) + parseInt(b);
                }, 0 );
        ...
        $('#purchase-filter').prependTo(dtWrapper);
        customizeDatatable();
        ...
        $('.bs-date, #branch-id, #status-id').on('change', function() {
            refreshTable();
        });
